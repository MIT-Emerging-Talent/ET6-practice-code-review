...
class TestLengthOfLongestSubstring(unittest.TestCase):
    ...
    def test_edge_cases(self):
        ...
        self.assertEqual(
            Solution().length_of_longest_substring("a"), 1
        )  # Single character
        self.assertEqual(
            Solution().length_of_longest_substring("aa"), 1
        )  # Repeating characters

    def test_boundary_cases(self):
        """Test boundary cases."""
        self.assertEqual(
            Solution().length_of_longest_substring("abcdefghij"), 10
        )  # All unique
        self.assertEqual(
            Solution().length_of_longest_substring("a" * 1000), 1
        )  # All same characters
        self.assertEqual(
            Solution().length_of_longest_substring("abc" * 333), 3
        )  # Repeating pattern
