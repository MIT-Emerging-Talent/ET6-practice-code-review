...
"""
    ...
    assert all(char in "01" for char in bin), "argument is not a binary number." #an assertion error raises if the argument is not a binary numbers
    rev_bin = bin[::-1] # reverse the binary number from left to right
    
    bin_list=list(map(int, rev_bin)) # convert the argue from a string to integer list
